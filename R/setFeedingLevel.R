...
#' @param feeding_level The feeding level to set for each species. Optional. A
#'   vector with one value for each species or a single value that is used for
#'   all species. If the argument is missing, the feeding level is taken from
#'   the `f0` column in the species_params object. If `f0` is not present, an
#'   error is thrown.
...
setFeedingLevel <- function(params, feeding_level) {
    ...
    new_intake_max[is.nan(new_intake_max)] <- Inf
    intake_max(params) <- new_intake_max
