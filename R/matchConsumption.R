...
#' increasing external encounter rates to compensate for changes in metabolic
#' loss.
#'
#' Thus the function also adjusts the external encounter rate to compensate for
#' the changed respiration rate. To do this the function assumes that both the
#' encounter rate and the metabolic respiration rate are given by power laws
#' with the same exponent `n`, so it sets the species parameter `p` to the same
#' value as `n`. A warning is issued if the exponent `p` had to be changed for
#' any species.
#'
