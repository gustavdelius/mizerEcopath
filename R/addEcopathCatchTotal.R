...
#' simplified "total" gear across all fleets. The `yield_observed` gear
#' parameter is set to the total catch for each species, and `catchability` is
#' initially set to the ratio of yield to biomass. This provides a starting
#' point for fishing pressure, but does not yet ensure steady-state or matched
#' yield.
#'
...
#'
#' In both cases, fishing is switched on with an initial effort of 1. However,
#' the result is not guaranteed to be at steady state, and catchability will
#' generally need further adjustment. You should call `matchCatch()` to refine
#' catchability and match the observed Ecopath yields.
#'
#' @details
#' The `sel_func` argument specifies the form of the length-based selectivity
#' function:
#'
...
#'
#' When `sel_func = "double_sigmoid_length"`, the additional columns `l50_right`
#' and `l25_right` are automatically added to the gear parameters, and are
#' calculated from the species parameter`max_observed_length` if it exists or
#' otherwise from the species parameter `w_max`.
#'
#' @param params A [`MizerParams`] object created with species parameters,
#'   typically including output from `addEcopathParams()`.
#' @param ecopath_catch The Ecopath Catch data frame
#' @param sel_func A string indicating the selectivity function to use. Either
#'   `"sigmoid_length"` (default) or `"double_sigmoid_length"`.
...
addEcopathCatchTotal <- function(params, ecopath_catch,
                                 sel_func = c("sigmoid_length",
                                              "double_sigmoid_length")) {
    sel_func <- match.arg(sel_func)
