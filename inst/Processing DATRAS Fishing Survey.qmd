...
cs_survey_final<-cs_survey%>%
    ...
    mutate(
        weight=a*length^b,
        scaled_biomass= (1e-6*weight*catch)/617499.7/(max(clean_data$Year)-min(clean_data$Year)+1),
        scaled_catch=catch/617499.7/(max(clean_data$Year)-min(clean_data$Year)+1))%>%
        select(-a, -b)
