...
Below we load the observed catch-at-length data, preprocess it, then run `matchCatch()` to fit both the single‐sigmoid and double‐sigmoid models
...
This section helps diagnose which starting parameters are associated with optimisation failures or poor fits.
By comparing the values of input multipliers between "good" and "outlier" runs, we can identify what ranges of input values (e.g. high natural mortality or extreme selectivity ratios) tend to lead to problems.
This guides refinement of parameter bounds and may reveal whether the optimiser struggles to disentangle certain effects — especially relevant for natural mortality estimation.
