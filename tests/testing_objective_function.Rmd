...
# Plot Original Single‐sigmoid Selectivity
Visualise the initial logistic (ascending) selectivity curves for each species. These are plotted before any matching is performed.

```{r plot-single-sigmoid}
gp <- gear_params(params_ss)

plot(NA,
     xlim = c(0, 1.6 * max(gp$l50, na.rm = TRUE)),
     ylim = c(0, 1),
...
# Plot Original Double‐sigmoid Selectivity
We plot the dome-shaped selectivity for each species. Note that until `matchCatch()` runs, `l50_right` and `l25_right` might be `NA`, so any species without those defined will be skipped.

```{r plot-dome-sigmoid}
gp2 <- gear_params(params_dome)

plot(NA,
     xlim = c(0, 1.6 * max(gp2$l50_right, na.rm = TRUE)),
     ylim = c(0, 1),
     xlab = "Length (cm)", ylab = "Selectivity",
...
for (sp_row in split(gp2, rownames(gp2))) {
  i <- i + 1
  if (anyNA(sp_row$l50_right)) next  # skip if not yet defined
...
if (length(finite_runs) > 0) {
  ...
  hist(objective_values,
       main = "Objective Values Across Valid Starts",
...
- **Robustness**: Out of 30 random starts, most (27–30) converged to nearly identical negative log-likelihoods.
...
- **Recommendations**:
  ...
  3. If convergence issues persist, explore **profile likelihood** around `mu_mat` and `catchability` to assess identifiability.
