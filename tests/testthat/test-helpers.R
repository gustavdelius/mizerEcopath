...
species_params <- data.frame(
    ...
    production_observed = c(NA, NA, NA),
    consumption_observed = c(NA, NA, NA),
...
test_that("Function adds Ecopath parameters correctly", {
    ...
    expect_true(is.na(result$consumption_observed[3]))
    expect_true(is.na(result$production_observed[3]))
...
test_that("Function works with missing Ecopath parameters", {
    ...
    expect_error(
        ...
        "The following groups in species_to_groups are not included in ecopath_params: Group1"
    )
})
...
test_that("Function does not overwrite values for unmapped species", {
    ...
    species_params_with_values$production_observed[3] <- 200
    species_params_with_values$consumption_observed[3] <- 300
