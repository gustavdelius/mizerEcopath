...
$$
The first term is a more standard diffusion term and can be treated with a second-order scheme.
$$
...
$$
\hat{g}_i = \tilde{g}_i - \frac{\tilde{d}_{i+1/2}-\tilde{d}_{i-1/2}}{h}.
$$

For the growth term we use an upwind difference scheme, depending on the sign of $\hat{g}$:
...
$$
\begin{split}
\frac{1}{2h}\left(\tilde{d}_{i+1/2}\frac{n_{i+1}-n_i}{h}-\tilde{d}_{i-1/2}\frac{n_i-n_{i-1}}{h}\right) - \frac{|\hat{g}_i|n_i-\hat{g}^+_{i+1}n_{i+1}-\hat{g}^-_{i-1}n_{i-1}}{h} - \mu_i n_i &= 0.
\end{split}
$$
...
$$
\begin{split}
D_i=&-\left(\frac{\tilde{d}_{i+1/2}+\tilde{d}_{i-1/2}}{2h^2}+\frac{|\hat{g}_i|}{h}+\mu_i\right),\\
L_i&=\frac{\tilde{d}_{i-1/2}}{2h^2}+\frac{\hat{g}^-_{i-1}}{h},\\ U_i&=\frac{\tilde{d}_{i+1/2}}{2h^2}+\frac{\hat{g}^+_{i+1}}{h}.
\end{split}
$$
Now we automatically have $L_i>0, U_i>0$ and $D_i<0$. The condition $U_i+L_i\leq -D_i$ is satisfied if
$$
\frac{\tilde{d}_{i+1/2}+\tilde{d}_{i-1/2}}{2h^2}+\frac{\hat{g}^+_{i+1}+\hat{g}^-_{i-1}}{h}\leq \frac{\tilde{d}_{i+1/2}+\tilde{d}_{i-1/2}}{2h^2}+\frac{|\hat{g}_i|}{h}+\mu_i,
$$
which simplifies to $0\leq \mu_i$, which is always satisfied.

```{r}
stable_steady <- function(dtilde, gtilde, mu, n0, h) {
    # We have to be careful when translating from maths to code because
    # in the maths we indexed our grid points from 0 to N+1, where $N$ is
    # the number of interior points, but in R we index from 1 to N+2.
    # So for example \tilde{d}_i in the maths corresponds to dtilde[i+1] in R.
    
    # Number of interior points
    N <- length(dtilde) - 2
    
    # Calculate finite difference scheme coefficients
    # We calculate the midpoint values for diffusion
    # \tilde{d}_{i+1/2} = (\tilde{d}_i + \tilde{d}_{i+1}) / 2
    # where \tilde{d}_{i} = dtilde[i+1] = for i=0,...,N+1
    # dtilde_half[i] = \tilde{d}_{i+1/2} for i=1,...,N+1
    dtilde_half <- (dtilde[2:(N+1)] + dtilde[3:(N+2)]) / 2
    # Calculate the adjusted growth rate
    # \hat{g}_i = \tilde{g}_i - \frac{\tilde{d}_{i+1/2} - \tilde{d}_{i-1/2}}{h}
    # \tilde{g}_i = gtilde[i+1] = for i=0,...,N+1
    ghat <- gtilde - (dtilde_half[3:(N+2)] - dtilde_half[1:N]) / (2 * h)  # Adjusted growth rate
    U <- (dtilde / 2)[3:(N+2)]    # Upper diagonal
    L <- (dtilde / 2 + h * gtilde)[1:N]     # Lower diagonal 
    D <- (-dtilde - h * gtilde - h^2 * mu)[2:(N+1)]  # Main diagonal
