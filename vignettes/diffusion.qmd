...
```{r}
params <- validParams(celtic_params)
species <- "Herring"
w <- params@w

# Without diffusion
params_no_diffusion <- params
ps_no_diffusion <- mizer::steadySingleSpecies(params_no_diffusion, species)
n_steady_no_diffusion <- ps_no_diffusion@initial_n[species, ]

# Set nonzero diffusion rate
species_params(params)$d_over_g <- 0.15

# Set diffusion from d_over_g
# d(w) = d_over_g * g(w) * w
for (s in species_params(params)$species) {
    d_over_g <- species_params(params)[s, "d_over_g"]
    w <- params@w
    growth <- getEGrowth(params)[s, ]
    n <- params@species_params[s, "n"]
    g_0 <- growth[1] / w[1]^n
    d_0 <- d_over_g * g_0
    ext_diffusion(params)[s, ] <- d_0 * w^(n + 1)
}

# Solve for steady state using the upwind scheme
ps <- mizer::steadySingleSpecies(params, species)
n_steady <- ps@initial_n[species, ]
