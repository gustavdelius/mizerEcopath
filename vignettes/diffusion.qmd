...
\end{split}
$$
Substituting this into the ODE and multiplying by $h^2$ gives us the linear system
$$
...
This scheme is known to be stable if $U_i,L_i>0$, $D_i<0$ and $U_i+L_i\leq -D_i$. These stability conditions are automatically satisfied if $\tilde{d},\tilde{g}$ and $\mu$ are all positive. 

We implement this in the function `solve_steady_state_upwind` as follows:
...
Nx <- solve_steady_state_upwind(dtilde, gtilde, mu, n0, h)
```
We can now plot the result.
```{r}
N <- length(Nx)
plot(x[2:(N+1)], log(Nx), type = "l", xlab = "log weight", ylab = "log abundance density")
lines(x[2:N+1], log(initialN(params)[species, 2:N+1]*w[2:N+1]), col = "red", lty = 2)
```
