...
stable_steady <- function(dtilde, ghat, mu, n0, h) {
    ...
    n <- solve_double_sweep(U[1:(N-1)], L[2:N], D, b)
...
A x = b
$$
where $A$ is a tridiagonal matrix with lower diagonal $L$, main diagonal $D$, and upper diagonal $U$.

The Thomas algorithm proceeds as follows:

- Let $\alpha$ be the modified upper diagonal (length $N-1$), $\beta$ the modified right-hand side (length $N$), and $d$ the modified main diagonal (length $N$).
- Initialize $\alpha_1 = U_1$, $\beta_1 = b_1$, $d_1 = D_1$.
- For $i = 2, \ldots, N$ (forward sweep):
$$
    m = \frac{L_{i-1}}{d_{i-1}} \\
    d_i = D_i - m \alpha_{i-1} \\
    \alpha_i = U_i \text{ (for } i < N\text{)} \\
    \beta_i = b_i - m \beta_{i-1}
$$
- After the forward sweep, perform back substitution:
    - $x_N = \beta_N / d_N$
    - For $i = N-1, \ldots, 1$:
$$
    x_i = \frac{\beta_i - \alpha_i x_{i+1}}{d_i}
$$

This yields the solution vector $x$.
...
solve_double_sweep <- function(U, L, D, b) {
    ...
    N <- length(D)
    if (length(U) != N - 1 || length(L) != N - 1 || length(b) != N) {
        stop("Dimensions of U, L, D, and b do not match for tridiagonal system.")
    }
    # Use alpha for modified upper diagonal, beta for modified right-hand side
    alpha <- U
    beta <- b
    dp <- D
    ...
    for (i in 2:N) {
        m <- L[i - 1] / dp[i - 1]
        dp[i] <- dp[i] - m * alpha[i - 1]
        beta[i] <- beta[i] - m * beta[i - 1]
    }
...
```{r}
# Plot the solution at several time points using ggplot2
library(tidyr)
library(dplyr)
library(ggplot2)

# Select time points to plot
plot_steps <- c(1, 2, 3, 21, 51, 101)
...
plot_df <- data.frame(
  ...
  Time = factor(rep(plot_labels, each = length(w)), levels = plot_labels)
)
