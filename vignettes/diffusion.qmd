...
species <- "Herring"
sps <- species_params(params)[species, ]
#ext_encounter(params) <- ext_encounter(params) * 0.1
#ext_mort(params) <- ext_mort(params) * 0.1
n <- sps$n
w <- w(params)
x <- log(w / w[1])
h <- x[2] - x[1]
...
# Plot
p <- ggplot(plot_df, aes(x = Length, y = Abundance*exp(t/2), color = Time)) +
