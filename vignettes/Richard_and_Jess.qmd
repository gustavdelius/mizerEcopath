...
`matchCatch()` adjusts, per species, the selectivity parameters $l_{50}$ and $l_{25}$ of both gears, the catchability of the commercial gear, the growth-diffusion coefficient `D_ext` and the external mortality coefficient `z_ext`, by minimising
...
data.frame(species = species_params(params)$species,
           reproduction_level = round(reproduction_level(params), 3)) |>
