...
The point of solving it this way is that the energy available for growth and reproduction, $E_r$, is held fixed. The fish now have a realistic physiology, but they grow exactly as they did before, so the size-distribution fit from stage 2 is not disturbed. The `steady()` calls afterwards simply let the model settle.
...
The same structure appears again in stage 6: the resource feedback is linear in $1 - \texttt{resource\_level}$. The two multiply along the food loop, and both are free in exactly the sense described in the introduction.
...
A few residual warnings about negative external *encounter* rates remain (Megrim, Hake): for these the diet matrix says they get more food from the modelled species than the model gives them in total, and the shortfall is clipped at zero.
...
The resource is then cut off at 1 g. This is not just realism about what counts as plankton; it is also what makes the next step work. Cutting the resource off small means that a large predator gets a negligible fraction of its food from the resource, so its large sizes stop constraining how much resource interaction it is allowed to have.
...
#| eval: false
#| code-fold: show
saveParams(params, "inst/params_05_Richard_and_Jess.rds")
```
...
All nine now sit within 7% of their target, against five of nine in the previous version of this document. It is worth being precise about where that improvement comes from, because the two changes are easily conflated.

Most of it is the change of target. Targeting $F_{MSY}$ rather than the fitted $F$ removed the two failures at the ends: hake's peak no longer has to be dragged *up* to 0.37 (see @sec-caveats), and herring and blue whiting no longer have targets so low that no reproduction level can reach them. That is a change in what we are asking for, not an improvement in the model.

What stages 3 and 7 contributed is narrower and real: cod and whiting were the two species whose peaks could not be brought down to their $F_{MSY}$ by any reproduction level, and raising whiting's feeding level and weakening cod's cannibalism is what brought them to 0.93 and 1.06. Without those two changes both would still be pinned at the reproduction floor with their peaks 16% and 26% too high.

The Ecopath calibration survives all of it. Against the model built with $f = 0.6$ throughout and full cannibalism, every species' $Q/B$, $P/B$ and biomass agree to better than 0.1%.
...
**The interactive and scripted routes disagree.** Setting those three production values in the tuning gadget and setting them by script give different models:

``` r
compareParams(pm, pt)
```

The discrepancy is not currently understood and is worth resolving, since it means one of the two routes is doing something other than what it appears to.

**Hake's earlier failure was an artefact of the target, not a finding.** The previous version of this document reported that no reproduction level could bring hake's yield peak up to the fishing mortality it currently experiences, and read that as the model saying hake is overfished. With $F_{MSY} = 0.24$ against a fitted $F$ of 0.37, the target is now *below* the current $F$ and hake converges at a reproduction level of 0.14 without difficulty. The earlier conclusion should not be carried forward: it was a statement about the criterion, not about hake.
...
**The** $F_{MSY}$ targets are a long way from the fitted fishing mortalities. Targeting $F_{MSY}$ instead of the current $F$ removes the old problem that lightly fished species could not be pinned down, but it replaces it with an extrapolation:
...
**The diet matrix does not fit everywhere.** Megrim and Hake still end up having their external encounter rate clipped at zero, meaning the diet matrix asks for more predation on modelled species than the model can supply.
...
**The catchability bug in the sweep script is fixed.** An earlier version of `inst/create_model_from_Richard_and_Jess.R` read the catchabilities from the raw `gp` data frame, where every species carries the placeholder 0.2, rather than the values `matchCatch()` fitted. The script now reads `gear_params(params)`, and with $F_{MSY}$ as the target the fitted catchabilities are used only for Plaice. Recorded results from before that change are not comparable.
