...
This document builds a nine-species mizer model of the Celtic Sea from data assembled by Richard and Jess, and then shows what the resulting model looks like. It is meant to explain the *method* to Richard and Jess: what each step does, why it is needed, and where the judgement calls are.
...
The final model is the model `psr` built above — which now includes the cannibalism reallocation of @sec-cannibalism — plus the reproduction levels from @sec-repro. Exactly this object, rebuilt by running the stages above, is saved as `inst/params_allometric_final_Richard_and_Jess.rds`, so every figure and table below can be reproduced without re-running the build. Its companion for the von Bertalanffy model of `vignettes/Richard_and_Jess_vB.qmd` is `inst/params_vB_final_Richard_and_Jess.rds`. See @sec-caveats for the older files that this one supersedes.
...
With all eighteen panels shown it helps to know which to look at hardest. Scoring each by the excess multinomial negative log likelihood per fish — the fitted misfit above the best any model could do on those raw bins, so that 0 is a perfect fit and the number is comparable across species — gives:

|   | worst | middling | best |
|----|----|----|----|
| **commercial** | Herring 0.191, Cod 0.134 | Hake 0.116, Megrim 0.108, Plaice 0.100 | Whiting 0.012, Haddock 0.016, Sole 0.033 |
| **survey** | **Blue whiting 0.954**, Plaice 0.410 | Hake 0.130, Haddock 0.123, Cod 0.090 | Sole 0.022, Whiting 0.031, Megrim 0.049 |

Three panels are worth dwelling on.

**Blue whiting's survey panel is by far the worst fit in the model**, at 0.954 — an order of magnitude worse than the median and on the species with the largest sample in the data set, so it is not noise. The observed distribution is a sharp hump at about 17 cm that has largely died away by 25 cm, and the model produces a curve that keeps rising through the observed mode and then decays far too slowly. The reason is structural: the survey gear has a `sigmoid_length` selectivity, which increases monotonically with size, so the only way for the *modelled* catch to fall away above 17 cm is for the abundance itself to fall away that steeply. It cannot, and the fit compromises by putting the rise in the wrong place. Plaice (0.410) is the same story more mildly, and Megrim's residual 0.049 is a weaker version again.

That is worth naming because it is not a failure of the growth model: the companion von Bertalanffy vignette shows the same three species misbehaving in the same way, and fixes them by giving those three *survey* gears a dome-shaped `double_sigmoid_length` selectivity, which drops Blue whiting's survey misfit from 0.954 to 0.096 and the mean over all species and gears from 0.196 to 0.060. Since the survey gear has catchability $10^{-12}$ and `yield_weight = 0`, that is a change to the observation model alone and would work here unchanged. The cost is that the survey then constrains those species' mortality less tightly, because a fall-off that the spectrum was being forced to explain is handed to the selectivity instead.

**Herring is the worst commercial fit** (0.191). Its observed commercial distribution is a narrow spike at 26 cm on a species whose asymptotic length is only about 30 cm, and a rising sigmoid on a smooth spectrum cannot make a peak that sharp. Herring is also the species whose $F_{MSY}$ target is the most extreme extrapolation (@sec-caveats), so it is the weakest link on two independent counts.

**Cod fits badly for a quite different reason** — small numbers. Its observed distribution is visibly the noisiest in the set because it rests on about 1900 fish against a quarter of a million for Blue whiting, so most of its 0.134 is irreducible scatter rather than model error.
...
The diet composition changes with predator size even though the diet matrix that was matched is a single size-averaged number per predator–prey pair: the size structure of the diet comes out of the predation kernel and the abundances, not out of the data. So the transitions in these panels are a prediction rather than an input, and they are the part of the diet the Ecopath data does not pin down.

The dominant impression is also correct and worth stating plainly: **this is a model in which the fish mostly do not eat each other.** Green (resource) and grey (external) fill almost every panel, because the `other` column of the diet matrix is large — most of what these nine species eat is not one of the nine. Named prey appears only in the largest sizes of a few predators: Blue whiting for Megrim, Hake and Cod, Herring for Whiting, and small contributions of Hake and Plaice to Cod. Cod is the most connected predator, with 17.8% of its consumption coming from modelled species, and Whiting next at 10.1%.

Every panel is smooth, which is worth noting only because the companion von Bertalanffy vignette's is not: there, the two species with a negative `t0` receive a juvenile growth subsidy that is switched off abruptly at `w_mat`, and their diet composition steps discontinuously as a result. `newAllometricParams()` gives every species a single power-law encounter rate over its whole size range, so nothing of that kind happens here.
...
**External mortality is the largest single source for every species.** It runs from 34% (Plaice) to 96% (Blue whiting) of the biomass-weighted total, and at larval sizes it is essentially 100% for all nine. That is not so much a modelling choice as the arithmetic of the diet matrix: predation mortality can only come from the nine modelled predators, and @fig-diet has just shown how small a part of anyone's diet they are.

This matters well beyond bookkeeping, and it is what makes stages 7 and 8 hard. External mortality is a **fixed rate** — it cannot fall when a species is fished down. So the compensatory mechanisms that hold a yield curve up, chiefly the release of juveniles from predation, are acting on the minority of the mortality budget, and the reproduction level has to do most of the work. It is also the general form of the argument made for cod in @sec-cannibalism.

**Predation is a hump over juveniles, not a background.** For every species the coloured band peaks between about 7 and 70 g and then collapses, because mizer's predation kernel means a predator eats prey one or two orders of magnitude lighter than itself, so the sizes exposed to the modelled predators are the small ones. The peaks are dramatic — 96% of Herring's mortality at 7 g is predation, and 95% of Hake's at 9 g — but they are confined to a size range that carries almost no biomass and no yield. That is why the biomass-weighted column and the peak column differ by up to a factor of thirty, and it is the biomass-weighted one that governs how a species responds to fishing.

**Two entries in the figure carry a lot of the model's behaviour.** Half of Plaice's mortality at 65 g is predation by Cod, which is the largest single interaction in the model and makes Plaice the second most predation-limited species overall (12%). And the blue band across the top of Cod's own panel is the cannibalism of @sec-cannibalism, still supplying up to 12% of Cod's mortality after being weakened to $\lambda = 0.1875$ — a reminder that what was reallocated was five sixths of a term that was doing real work.

**Fishing repeats the extrapolation caveat in visual form.** Red reaches 82% of Cod's mortality at large sizes and 55–86% for Megrim, Haddock, Whiting, Hake, Plaice and Sole, but peaks at only 3% for Herring and 4% for Blue whiting. Those two are essentially unfished in this model, and they are precisely the two whose $F_{MSY}$ targets sit 70 and 27 times above their fitted $F$ (@sec-caveats). The figure shows how far outside its calibrated state the model is being asked to speak for them.
...
**The red line is not approached.** The tallest bar is Herring's at 0.018, a factor of 55 below 1, and the shortest is Cod's at $9.7\times10^{-5}$. So nowhere in the steady state is the model demanding more larval biomass than the spawning stock can physically produce, which is the check the red line exists to perform.

**The spread is about two and a half orders of magnitude, and most of it is fisheries-realistic.** Cod at $10^{-4}$ is a large, long-lived, heavily fished species putting an enormous amount of energy into reproduction for very few survivors, which is what egg-to-recruit survival looks like for a cod stock. Herring at the other end is small, short-lived and effectively unfished here, so much less of its investment is wasted.

**The bars should not be read as reproductive biology alone, though.** Megrim and Haddock have the two highest raw `erepro` values (0.040 and 0.020) but sit in the middle of the realised ranking, because @sec-repro gave them reproduction levels of 0.901 and 0.823 and so discounted them tenfold and sixfold. Cod and Whiting, at the reproduction floor, have bars that are essentially their raw `erepro`. For the four species with a high reproduction level, most of the bar height is the density dependence that the yield-peak criterion demanded rather than anything about their eggs — and @sec-caveats records how weakly that criterion pins it down.
...
**There are several saved model files and only one of them is this model.** The one to use is `inst/params_allometric_final_Richard_and_Jess.rds`, which is the object built by this document. Two older files are still in `inst/` and neither matches it:

- `inst/params_Richard_and_Jess.rds` has `R_max = Inf` for eight of the nine species, so it is really the stage-6 model, before any density dependence in reproduction.
- `inst/params_05_Richard_and_Jess.rds` (the suffix records the resource level) does carry finite `R_max`, but its reproduction levels are from a different pass of the stage-8 sweep than the ones recorded in @sec-repro: Herring 0.202 against 0.379, Hake 0.257 against 0.142, Blue whiting 0.010 against 0.079, and smaller differences for Haddock and Plaice. Since @sec-repro notes that the two sweeps did not agree, this is most likely the first sweep rather than the second.

The document nevertheless reconstructs the model from `psr` and the recorded reproduction levels rather than loading any file, so that what is shown below is always what the code above produces.

Note also that the `.rds` files are *not* excluded by `.gitignore` — the repository has no `*.rds` pattern — so these objects are committed and are available to anyone cloning it. That is convenient but it means a 290 kB binary is versioned every time a model is re-saved.
