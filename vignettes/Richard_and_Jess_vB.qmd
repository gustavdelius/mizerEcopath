...
The final model is the stage-7 model built above plus the reproduction levels from @sec-repro. We reapply them here rather than loading `inst/params_vB_final_Richard_and_Jess.rds` because the repository ignores `*.rds`, so collaborators cloning it do not have that file.
...
Megrim is shown because it is the species whose diet the model still represents in full: after @sec-cod, Cod's fish predation has been reallocated, and Whiting's reduced by a third.
...
**Megrim needs `erepro` > 1.** At $r_l = 0.907$ `setBevertonHolt()` warns that Megrim requires a reproductive efficiency above 1 — the model is being asked for more recruits than the spawning stock can physically produce. Megrim's reproduction level should be treated as an upper bound rather than an estimate.
